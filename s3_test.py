					...
					if p.status_code != 200:
						...
						for param in initialLoad:
							for payload in payloads:
								...
								print "----------------------------------------"
								if (((length > initialLength) and (status == 200)) and(abs(length - initialLength) > r.content.count(payload)*len(payload)) or ((trip - initialTrip) > 5)) and (parsedNewEndingUrl == parsedInitialEndingUrl) and (self_checkStillLoggedIn(loginpayload,r.content)) and (not self_gotsqlsyntaxerror(r.content)):
									initialUrl = copy.deepcopy(urls)
